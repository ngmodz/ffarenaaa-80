...
const ProfileTabs = ({
  ...
  return <Tabs defaultValue="joined" className="w-full">
      <div className="flex flex-col gap-4">
        ...
        <div className="relative w-full">
          ...
          <Input placeholder="Search tournaments..." className="pl-9 bg-[#111827] border-gaming-border text-white w-full" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
        
        <div className="flex gap-2 mb-2 flex-wrap">
          <Badge onClick={() => handleFilterChange('upcoming')} className={`cursor-pointer ${activeFilter === 'upcoming' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-[#111827] hover:bg-[#374151] text-[#A0AEC0]'}`}>
            Upcoming
          </Badge>
          <Badge onClick={() => handleFilterChange('ongoing')} className={`cursor-pointer ${activeFilter === 'ongoing' ? 'bg-gaming-accent hover:bg-gaming-accent/90' : 'bg-[#111827] hover:bg-[#374151] text-[#A0AEC0]'}`}>
            Live
          </Badge>
          <Badge onClick={() => handleFilterChange('completed')} className={`cursor-pointer ${activeFilter === 'completed' ? 'bg-gray-600 hover:bg-gray-700' : 'bg-[#111827] hover:bg-[#374151] text-[#A0AEC0]'}`}>
            Completed
          </Badge>
          {activeFilter && <Badge onClick={() => handleFilterChange(null)} variant="destructive" className="cursor-pointer">
              Clear Filter
            </Badge>}
        </div>
      </div>
      ...
      <TabsContent value="joined" className="mt-4">
        <TournamentActivityList tournaments={getFilteredTournaments(mockJoinedTournaments)} type="joined" />
      </TabsContent>
      
      <TabsContent value="hosted" className="mt-4">
        <TournamentActivityList tournaments={getFilteredTournaments(mockHostedTournaments)} type="hosted" />
      </TabsContent>
      
      <TabsContent value="winnings" className="mt-4">
        <TournamentActivityList tournaments={getFilteredTournaments(mockWinnings)} type="winnings" />
      </TabsContent>
    </Tabs>;
};
